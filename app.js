...
// mongoose.connect("mongodb+srv://abhishek:abhishek@divij.tlfyj.mongodb.net/myFirstDatabase?retryWrites=true&w=majority");
...
app.post("/user-roll-get", function(req, res){
    ...
    Student.find({'REGNO': req.body.regno}, function(err, lancer){
        if(err){
            ...
            lancer.push(req.body.regno);
            console.log(lancer);
            console.log(req.body.regno);
            
            res.render("student-info",{lancer: lancer});
        }
    });

});
app.get("/user-roll-get1",function(req,res){
    res.render("user-roll-get1");
});
app.post("/user-roll-get1", function(req,res){
    student_data.name=req.body.regno;
    let a=req.body.regno;
    student.push(student_data);
    res.render({a:a});
    console.log(student);
    res.redirect("/user-roll-get1");
});
    ...
    dataModel.find((err,data)=>{
             if(err){
                console.log(err)
                 ...
                 if(data!=''){
                     res.render('my-user-profile',{data:data});
                 }else{
                     res.render('my-user-profile',{data:''});
                 }
             }
    })
});
